...
//
// Replaces the old PostgreSQL connection (src/lib/db.ts). Provides:
...
//
// The admin client authenticates lazily on first use and auto-refreshes
// its token. Building `next build` won't crash because no network call
// happens until the first property access triggers authentication.
...
export function getAdminClient(): PocketBase {
  if (!_adminClient) {
    ...
    // Disable auto-cancellation: server-side operations (sync, match)
    // run concurrent requests that must not cancel each other.
 ...
 * Used for per-request auth validation in the DAL.
 */
